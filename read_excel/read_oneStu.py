...
f0 = open('./oneStudent/oneWeek.txt', 'w', encoding='utf-8')
'''
#读取学生name在week周的课前、课后和每周总结数据
def choiceText(name,week):
	#课前预习筛选
	for i in range(nrows1):
		a = sheet1.cell_value(i,2)#获取姓名
		b = sheet1.cell_value(i,4)#获取周次
		c = sheet1.cell_value(i,6)#获取评论文本
		if(a == name and b == week):
			print("课前：",c)
			f0.write(c)
			f0.write('\n')
	#课后作业筛选
